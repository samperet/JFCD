...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const u = safeParse(localStorage.getItem(KEY_USERS), null);
    if (u) setUsers((prev) => ({ ...prev, ...u }));

    const m = safeParse(localStorage.getItem(KEY_MSGS), null);
  ...
  // Persist.
  useEffect(() => {
    if (hydrated) localStorage.setItem(KEY_MSGS, JSON.stringify(messages));
  }, [messages, hydrated]);
  useEffect(() => {
    if (hydrated) localStorage.setItem(KEY_USERS, JSON.stringify(users));
  }, [users, hydrated]);
  useEffect(() => {
    if (!hydrated) return;
    if (currentUser) localStorage.setItem(KEY_CURRENT, currentUser);
